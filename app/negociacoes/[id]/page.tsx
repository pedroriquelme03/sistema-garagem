...
export default function VerNegociacao() {
  ...
  if (!negociacao) return <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8"><p className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">Negociação não encontrada.</p><Link href="/negociacoes" className="mt-4 inline-block text-sm font-semibold text-brand-600">← Negociações</Link></div>;

  return <div className="mx-auto max-w-3xl px-4 py-7 sm:px-6 lg:px-8">
    <header className="mb-7 flex items-end justify-between"><div><p className="text-sm font-medium text-brand-600">Negociação</p><h1 className="mt-1 text-3xl font-bold text-slate-950">{negociacao.tipo}</h1><p className="mt-1 text-sm text-slate-500">Criada em {new Date(negociacao.criadoEm).toLocaleString("pt-BR")}</p></div><Link href="/negociacoes" className="text-sm font-semibold text-slate-600">← Negociações</Link></header>
    ...
    <div className="space-y-6">
      ...
      {cliente && veiculo && <Link href={`/simulacoes?clienteId=${cliente.id}&veiculoId=${veiculo.id}&valor=${encodeURIComponent(negociacao.valor)}`} className="inline-block text-sm font-semibold text-brand-600">Simular financiamento →</Link>}
