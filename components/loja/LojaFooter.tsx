...
import { consultaDoMapa, embedGoogleMaps, linkGoogleMaps, nivelVitrine } from "@/lib/loja-publico";
...
export default function LojaFooter() {
  ...
  return (
    <footer className={escuro ? "border-t border-white/10 bg-patio-night text-white" : "border-t border-patio-sand bg-patio-cream"}>
      <div className={`mx-auto grid max-w-6xl gap-10 px-4 pb-8 pt-12 sm:px-6 ${master ? "md:grid-cols-[1.6fr_1fr_1fr]" : "md:grid-cols-[1.4fr_1fr_1fr]"}`}>
        <div>
          {loja.logoSrc
          ...
          <p className={`mt-4 max-w-sm text-sm leading-6 ${escuro ? "text-white/60" : "text-patio-mute"}`}>
            {master
              ? "Showroom da loja: o estoque do site é o mesmo do chão. Sem anúncio fantasma."
              : pro
                ? "Seminovos no pátio. O que está no site está no chão — e some quando vende."
                : "Pátio aberto: o estoque do site é o mesmo do chão da loja. Sem anúncio fantasma, sem marca que não tem carro."}
          </p>
        </div>
        <div>
          ...
          <div className="mt-3 flex flex-col gap-2 text-sm">
            ...
            {pro ? (
              <>
                <Link href="/loja#financiamento" className={escuro ? "text-white/70 hover:text-patio-cobalt" : "text-patio-mute hover:text-patio-cobalt"}>Financiar</Link>
                <Link href="/loja#avaliar" className={escuro ? "text-white/70 hover:text-patio-cobalt" : "text-patio-mute hover:text-patio-cobalt"}>Vender meu carro</Link>
              </>
            ) : null}
            {master ? (
              <>
                <Link href="/loja#a-loja" className={escuro ? "text-white/70 hover:text-patio-cobalt" : "text-patio-mute hover:text-patio-cobalt"}>A loja</Link>
                <Link href="/loja#financiamento" className={escuro ? "text-white/70 hover:text-patio-cobalt" : "text-patio-mute hover:text-patio-cobalt"}>Financiamento</Link>
              </>
            ) : null}
        ...
        <div>
          ...
          {loja.facebook || loja.instagram ? (
            ...
            <p className={`mt-3 text-sm ${escuro ? "text-white/55" : "text-patio-mute"}`}>
              {master || pro ? loja.cidade : "Cadastre Instagram e WhatsApp nas configurações do Garagem Pro."}
            </p>
          )}
        </div>
      </div>
      ...
      <div className={`border-t py-4 text-center text-xs ${escuro ? "border-white/10 text-white/40" : "border-patio-sand text-patio-mute"}`}>
        {master || pro ? `© ${loja.nome}` : `${loja.nome} · vitrine do Garagem Pro`}
      </div>
    </footer>
  );
}
